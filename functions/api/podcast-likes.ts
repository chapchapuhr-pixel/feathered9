...
export const onRequestPost: PagesFunction = async ({ request, env }) => {
  try {
    ...
    if (existing?.id) {
      ...
      return Response.json(
        { success: true, action: "unliked", likes_count: Number((countRow as any)?.likes_count || 0), liked_by_me: 0 },
    ...
    return Response.json(
      { success: true, action: "liked", likes_count: Number((countRow as any)?.likes_count || 0), liked_by_me: 1 },
...
export const onRequestGet: PagesFunction = async ({ request, env }) => {
  try {
    ...
    if (!episode_id) {
      return Response.json({ error: "episode_id is required" }, { status: 400, headers: cors });
    }
    ...
    return Response.json(
      { episode_id, likes_count: Number((countRow as any)?.likes_count || 0), liked_by_me },
      { status: 200, headers: cors }
    );
  } catch (e: any) {
    return Response.json(
      { error: e?.message || "Failed to fetch likes" },
      { status: 500, headers: cors }
    );
  }
};
