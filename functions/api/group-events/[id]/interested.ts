...
export const onRequestPost: PagesFunction<Env> = async ({ request, env, params }) => {
  try {
    ...
    if (isAdd) {
      ...
      if (!already) {
        await env.DB
          .prepare(`INSERT INTO group_event_interested (group_event_id, user_id) VALUES (?, ?)`)
          .bind(finalEventId, userId)
          .run();
        ...
        if (eventOwnerId && eventOwnerId !== userId) {
          try {
            await createNotification(
              ...
              "group_event",
              finalEventId,
              `group_event:${finalEventId}:interested`,
              `is interested in your group event "${event.title || 'Event'}"`
            );
          } catch (_) {}
        }
      }
